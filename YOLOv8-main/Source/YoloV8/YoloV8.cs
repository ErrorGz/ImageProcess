...
public class YoloV8 : IDisposable
{
    ...
    private Tensor<float> Preprocess(Image<Rgb24> image)
    {
        ...
        float[] inputArray = new float[modelSize.Width * modelSize.Height * 3];
        var TotalPixel = modelSize.Width * modelSize.Height;
        Parallel.For(0, image.Height, y =>
        {
            image.ProcessPixelRows(row =>
            {
                ...
                {
                    var R_offset = y * modelSize.Width + x;
                    var G_offset = TotalPixel + y * modelSize.Width + x;
                    var B_offset = TotalPixel * 2 + y * modelSize.Width + x;
