...
namespace ImageLabel
{
    public partial class frmPlay : Form
    {
        ...
        YOLOv5<EmptyYoloModel> yoloModel;
        List<YoloLabelModel> yoloLabels;
        ...
        public frmPlay(YOLOv5<EmptyYoloModel> yolo, List<YoloLabelModel> listLabel)
        {
            InitializeComponent();
            //InitKF(kf);
            yoloModel = yolo;
        ...
        private void backgroundWorker1_ProgressChanged(object sender, System.ComponentModel.ProgressChangedEventArgs e)
        {
            try
            {
                if (flagFormClosing == false)
                {
                    ...
                    var img_mat = rd.img_detect;
                    img_mat = ImageSharpExtensions.Resize(img_mat, PictureBox1.ClientSize.Width, PictureBox1.ClientSize.Height);
                    var img_bitmap = img_mat.ToBitmap();
                    PictureBox1.Image = img_bitmap;
        ...
        private void PutImageToPictureBox(PictureBox pb, Image img)
        {
            //if (pb.Image == null)
        ...
        private void 打开图片ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                Mat img_mat = Mat.FromStream(fs, ImreadModes.Color);
                var predictions = yoloModel.Predict(img_mat);

                foreach (var prediction in predictions) // iterate predictions to draw results
                {
                    double score = Math.Round(prediction.Score, 2);

                    Cv2.Rectangle(img_mat,
                        new Rect((int)prediction.Rectangle.X, (int)prediction.Rectangle.Y, (int)prediction.Rectangle.Width, (int)prediction.Rectangle.Height),
                        new OpenCvSharp.Scalar(255, 255, 0),
                        3);

                    var (x, y) = (prediction.Rectangle.X - 3, prediction.Rectangle.Y - 23);
                    var msg = $"{prediction.LabelId} ({score})";
                    Cv2.PutText(img_mat, msg, new OpenCvSharp.Point(x, y), HersheyFonts.HersheySimplex, 1.0f, new OpenCvSharp.Scalar(0, 0, 0));
        ...
        private void RunCaptureVideo(BackgroundWorker bgWorker)
        {
            ...
            var Font = new Font("Consolas", 32, GraphicsUnit.Pixel);
            var Frames = 0f;
            ...
            track.On出现 = (o) =>
            {
                var labelname = yoloLabels.Where(p => p.Id == o.Current_Prediction.LabelId).Select(o => o.Name).FirstOrDefault();
            ...
            track.On消失 = (o) =>
            {
                var labelname = yoloLabels.Where(p => p.Id == o.Last_Prediction.LabelId).Select(o => o.Name).FirstOrDefault();
            ...
            try
            {
                while (!bgWorker.CancellationPending)
                {
                    ...
                    foreach (var prediction in predictions) // iterate predictions to draw results
                    {
                        var labelname = yoloLabels.Where(o => o.Id == prediction.LabelId).Select(o => o.Name).FirstOrDefault();
                        var color = lib.Colors[prediction.LabelId % 32];
                        var score = Math.Round(prediction.Score, 2);

                        Cv2.Rectangle(img_mat,
                            new OpenCvSharp.Rect((int)prediction.Rectangle.X, (int)prediction.Rectangle.Y, (int)prediction.Rectangle.Width, (int)prediction.Rectangle.Height),
                            new OpenCvSharp.Scalar(color.R, color.G, color.B),
                            3);
                        (var cx, var cy) = (prediction.Rectangle.X + prediction.Rectangle.Width / 2, prediction.Rectangle.Y + prediction.Rectangle.Height / 2);
                        Cv2.Circle(img_mat, new OpenCvSharp.Point(cx, cy), 2, new OpenCvSharp.Scalar(color.R, color.G, color.B),3);

                        var msg = $"{prediction.LabelId}:{labelname} ({score})";
                        lib.PutText(img_mat, msg, (int)prediction.Rectangle.X, (int)prediction.Rectangle.Y);
                    }
                    ...
                    lib.PutText(img_mat, msg_TrainSecond, 0, 0);
                    #endregion



                    bgWorker.ReportProgress(0, new ReportData() { img_source = img_source, img_detect = img_mat, predictionModels = predictions });
    ...
    {
        public Mat img_source;
        public Mat img_detect;
        public List<YoloPrediction> predictionModels;
